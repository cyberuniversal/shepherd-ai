"""Validate the complete unreviewed MultiUAV intervention dataset."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--review-packet",
        type=Path,
        default=ROOT / "reports" / "multiuav_intervention_review_v1.csv",
    )
    parser.add_argument(
        "--generation-summary",
        type=Path,
        ...
        / "datasets"
        / "multiuav_plat"
        / "intervention_dataset_summary_v1.json",
    )
    ...
    if len(eligibility.get("records", [])) != EXPECTED_TASK_COUNT:
        raise ValueError("eligibility manifest must contain all source tasks")
    if dataset["metadata"].get("source_archive_sha256") != archive_hash:
        raise ValueError("intervention dataset is not bound to the pinned source")
    if dataset["metadata"].get("eligibility_sha256") != eligibility_hash:
        raise ValueError("intervention dataset is not bound to task eligibility")
    ...
    source_by_id = _load_selected_source(args.archive, eligible_ids)
    ...
    if result["eligible_clusters"] != 1_473 or result["cases"] != 7_365:
        raise ValueError("validated dataset does not match recorded eligibility counts")
    ...
    output = {
        ...
        "summary": result,
        "claim_status": "unreviewed_full_dataset_validated_not_evaluation_data",
    }
