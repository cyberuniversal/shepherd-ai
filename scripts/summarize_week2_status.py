...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--span-intent-comparison", help="Span-derived intent path comparison JSON.")
    parser.add_argument("--asr-span-intent-impact", help="ASR-to-span-intent impact JSON.")
    parser.add_argument(
        "--hf-transcript-human-intent-accuracy",
        help="Imported Hugging Face transcript-intent accuracy JSON for human/reference transcripts.",
    )
    parser.add_argument(
        "--hf-transcript-asr-intent-accuracy",
        help="Imported Hugging Face transcript-intent accuracy JSON for ASR/Whisper transcripts.",
    )
...
def main() -> None:
    ...
    summary = build_summary(
        ...
        span_intent_comparison=_load_json(args.span_intent_comparison) if args.span_intent_comparison else None,
        asr_span_intent_impact=_load_json(args.asr_span_intent_impact) if args.asr_span_intent_impact else None,
        hf_transcript_human_intent_accuracy=(
            _load_json(args.hf_transcript_human_intent_accuracy)
            if args.hf_transcript_human_intent_accuracy
            else None
        ),
        hf_transcript_asr_intent_accuracy=(
            _load_json(args.hf_transcript_asr_intent_accuracy)
            if args.hf_transcript_asr_intent_accuracy
            else None
        ),
        source_paths={
            ...
            **({"span_intent_comparison": args.span_intent_comparison} if args.span_intent_comparison else {}),
            **({"asr_span_intent_impact": args.asr_span_intent_impact} if args.asr_span_intent_impact else {}),
            **(
                {"hf_transcript_human_intent_accuracy": args.hf_transcript_human_intent_accuracy}
                if args.hf_transcript_human_intent_accuracy
                else {}
            ),
            **(
                {"hf_transcript_asr_intent_accuracy": args.hf_transcript_asr_intent_accuracy}
                if args.hf_transcript_asr_intent_accuracy
                else {}
            ),
        },
...
def build_summary(
    *,
    ...
    span_intent_comparison: dict[str, Any] | None,
    asr_span_intent_impact: dict[str, Any] | None,
    hf_transcript_human_intent_accuracy: dict[str, Any] | None,
    hf_transcript_asr_intent_accuracy: dict[str, Any] | None,
    ...
    span_intent_summary = (span_intent_comparison or {}).get("summary", {})
    raw_span_intent = span_intent_summary.get("span_intent_assembly", {})
    hybrid_span_intent = span_intent_summary.get("hybrid_span_parser", {})
    asr_span_intent_summary = (asr_span_intent_impact or {}).get("summary", {})
    asr_hybrid_span_intent = asr_span_intent_summary.get("hybrid_span_parser", {})
    hf_transcript_human_summary = (hf_transcript_human_intent_accuracy or {}).get("summary", {})
    hf_transcript_asr_summary = (hf_transcript_asr_intent_accuracy or {}).get("summary", {})
    ...
    headline = {
        ...
        "raw_span_intent_exact_accuracy": raw_span_intent.get("exact_record_accuracy"),
        "raw_span_intent_field_accuracy": raw_span_intent.get("field_accuracy"),
        "hybrid_span_intent_exact_accuracy": hybrid_span_intent.get("exact_record_accuracy"),
        "hybrid_span_intent_field_accuracy": hybrid_span_intent.get("field_accuracy"),
        "asr_hybrid_span_intent_changed_records": asr_hybrid_span_intent.get("intent_changed_records"),
        "asr_hybrid_span_intent_changed_rate": asr_hybrid_span_intent.get("intent_changed_rate"),
        "hf_transcript_human_span_intent_exact_accuracy": hf_transcript_human_summary.get(
            "span_intent_assembly", {}
        ).get("exact_record_accuracy"),
        "hf_transcript_human_hybrid_intent_exact_accuracy": hf_transcript_human_summary.get(
            "hybrid_span_parser", {}
        ).get("exact_record_accuracy"),
        "hf_transcript_human_deterministic_intent_exact_accuracy": hf_transcript_human_summary.get(
            DETERMINISTIC_V3_SUMMARY_KEY, {}
        ).get("exact_record_accuracy"),
        "hf_transcript_asr_span_intent_exact_accuracy": hf_transcript_asr_summary.get(
            "span_intent_assembly", {}
        ).get("exact_record_accuracy"),
        "hf_transcript_asr_hybrid_intent_exact_accuracy": hf_transcript_asr_summary.get(
            "hybrid_span_parser", {}
        ).get("exact_record_accuracy"),
        "hf_transcript_asr_deterministic_intent_exact_accuracy": hf_transcript_asr_summary.get(
            DETERMINISTIC_V3_SUMMARY_KEY, {}
        ).get("exact_record_accuracy"),
    }
    ...
    return {
        ...
        "details": {
            ...
            "span_to_intent_paths": span_intent_summary,
            "asr_span_intent_impact": asr_span_intent_summary,
            "hf_transcript_intent_accuracy": {
                "human_transcript": hf_transcript_human_summary,
                "asr_transcript": hf_transcript_asr_summary,
            },
        },
        "interpretation": _interpretation(
            headline,
            hf_token_error_analysis,
            span_intent_comparison,
            asr_span_intent_impact,
            hf_transcript_human_intent_accuracy,
            hf_transcript_asr_intent_accuracy,
        ),
        "limitations": [
            ...
            "ASR span-to-intent impact compares model predictions on human and Whisper transcripts; it is not gold ASR intent accuracy.",
            "The strongest transformer result is text-command span extraction, not speech-to-mission performance.",
            "The hybrid span-to-intent result is post-hoc development on the same 10-record span test set, not a clean final benchmark.",
            "The 30-record transformer transcript-intent result is diagnostic because the model was not trained on audio_v2_holdout intent labels and the deterministic parser has same-batch development history.",
        ],
    }
...
def render_markdown(summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"| Raw trained-span intent exact accuracy | {_pct(headline['raw_span_intent_exact_accuracy'])} |",
        f"| Post-hoc hybrid span intent exact accuracy | {_pct(headline['hybrid_span_intent_exact_accuracy'])} |",
        f"| ASR-caused hybrid span-intent changes | {_count(headline['asr_hybrid_span_intent_changed_records'])} records |",
        f"| Transformer human-transcript intent exact accuracy | span {_pct(headline['hf_transcript_human_span_intent_exact_accuracy'])}; hybrid {_pct(headline['hf_transcript_human_hybrid_intent_exact_accuracy'])}; deterministic {_pct(headline['hf_transcript_human_deterministic_intent_exact_accuracy'])} |",
        f"| Transformer ASR-transcript intent exact accuracy | span {_pct(headline['hf_transcript_asr_span_intent_exact_accuracy'])}; hybrid {_pct(headline['hf_transcript_asr_hybrid_intent_exact_accuracy'])}; deterministic {_pct(headline['hf_transcript_asr_deterministic_intent_exact_accuracy'])} |",
        "",
        "## Interpretation",
        "",
    ]
...
def _interpretation(
    ...
    if span_intent_comparison:
        items.append(
            "The trained-span path now feeds a bounded span-to-intent bridge; the post-hoc hybrid repairs the current 10-record span-derived benchmark but still needs fresh validation."
        )
    if asr_span_intent_impact:
        items.append(
            "Existing ASR span-to-intent impact is limited to one raw constraint-string change in the 10-record local audio sample."
        )
    if hf_transcript_human_intent_accuracy and hf_transcript_asr_intent_accuracy:
        items.append(
            "On the 30-record audio_v2_holdout diagnostic benchmark, the imported transformer span path is currently much weaker than the deterministic parser for final intent JSON."
        )
    items.append(
        "The next research-useful improvement is targeted span-to-intent failure analysis and a fresh clean held-out evaluation set, not treating the post-hoc development batches as final benchmarks."
    )
    return items
...
DETERMINISTIC_V3_SUMMARY_KEY = "deterministic_v3"
...
def _count(value: Any) -> str:
    return str(value) if isinstance(value, int) else "not stated"
