...
|---|---|---|---|
| CP-25 | Pass correctness and smoke gates before the full GPU run. | partial for attempt 3 | Source, split, eligibility, alias, wiring, model-cache, synthetic-load, scoring-contract, unit-test, and cache/runtime gates pass. Resource preflight v2 admitted both frozen models against attempt-2 controls without loading either model or starting measurement. The cooldown-aware attempt-3 execution revision passes local tests, but its independently versioned RTX 3090 preflight v3 remains required before the full campaign. |
...
| CP-36 | Separate one deterministic accuracy run from three resource repetitions on 30 clusters. | partial | `accuracy_case_manifest_v1.json` approves 284 held-out clusters and 1,420 cases. One 5,680-row matrix per frozen model is complete and score-blind admission passes for all 11,360 rows. `resource_schedule_v1.json` binds 30 complete approved clusters, 150 cases, and 24 conditions to the frozen RTX 3090 protocol. Resource preflight v2 passed. Campaign attempt 1 failed before measurement with zero rows. Attempt 2 preserved one complete 150-row 3B/M3 condition, then failed before condition 2 measurement when the GPU baseline remained at 68 C against the frozen 60 C ceiling. No full resource repetition is admitted. All 24 attempt-3 configs are bound to cooldown-aware execution commit `6d0030094956626b17a9506d018e0fc077ca0ffd`; preflight v3 remains required before measurement. |
| CP-37 | Record the full safety, utility, fidelity, latency, token, memory, call, and GPU-energy trade-off. | partial | Accuracy scoring now records raw and post-gate unsafe proceed, decision utility, schema/endpoint/parameter/official-command fidelity, parse/backend failures, and containment for all 11,360 rows. Accuracy runs were intentionally not resource measurements. Attempt 2 contains one preserved raw resource condition, but the 24-condition campaign and all three complete repetitions remain pending; no partial resource comparison has been scored. |
...
| CP-39 | Checkpoint every row with compatible resume and complete raw packages. | partial | Schema-v3 config-bound JSONL writes, per-row `fsync`, duplicate/config-drift rejection, zero-call resume, durable progress callbacks, matrix completeness, and deterministic compact ZIP checkpoints are implemented. The preserved 3B and 7B archives each contain all 5,680 expected rows and pass hash, ZIP, config, and admission checks. Resource attempt 1 retained its zero-row failure package. Attempt 2 retains its complete 150-row first-condition package and second-condition pre-measurement failure package with verified hashes, but no complete resource campaign exists. |
...
| LP-47 | Thirty clusters, five variants, three resource repetitions. | partial; the final schedule contains 30 approved held-out clusters, all 150 five-variant cases, and 24 source-hashed condition orders. One raw 150-row condition is preserved from failed attempt 2, but no complete resource repetition or admitted comparison exists |
...
|---|---|---|
| DoD-53 | Record every data and model checksum. | partial; source/data artifacts, immutable Qwen revisions, cached snapshot inventories, both complete accuracy-run result/checkpoint hashes, and the failed attempt-2 resource package hashes are recorded. Complete resource-run checksums remain absent. |
...
| DoD-55 | Retain every expected method-case row, including failures and parse errors. | partial | Both sealed accuracy archives retain all 11,360 expected method-case rows. Complete-matrix admission retains failures and rejects missing, duplicate, unapproved, synthetic, or resource rows. Failed resource attempt 2 retains its 150 completed rows; the remaining full-campaign rows remain pending. |
...
resulting execution commit. The exact next gate is no-inference RTX 3090
preflight v3. The full attempt-3 campaign may start only after both frozen-model
summaries pass validation.
