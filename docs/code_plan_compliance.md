...
|---|---|---|---|
| CP-25 | Pass correctness and smoke gates before the full GPU run. | implemented | Source, split, eligibility, alias, wiring, model-cache, synthetic-load, scoring-contract, unit-test, cache/runtime, and RTX 3090 resource preflight gates passed. Resource preflight v2 independently admitted both frozen models against the attempt-2 execution controls without loading either model or starting measurement. |
...
raw package is preserved and unscored. The exact next gate is to make baseline
acquisition wait for both idle and thermal eligibility within the existing
600-second window, test that behavior, bind a fresh attempt to the resulting
commit, and repeat the no-inference RTX 3090 preflight before starting the full
campaign.
