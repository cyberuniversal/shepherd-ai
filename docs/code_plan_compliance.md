...
| CP-36 | Separate one deterministic accuracy run from three resource repetitions on 30 clusters. | partial | `accuracy_case_manifest_v1.json` approves 284 held-out clusters and 1,420 cases for one deterministic accuracy run per model. Final configs bind one 5,680-row accuracy matrix per model to commit `a64eeae2f0ec5508d17c20e2b25e1951f27f7947`; both preflights pass without resource monitoring. Resource configs remain separate: `resource_schedule_candidate_v1.json` selects 30 tasks and registers 24 model-method-repetition conditions; warm-up and final hardware bindings remain resource-only blockers. |
