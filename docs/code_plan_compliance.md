...
| LP-45 | Primary stage-wise versus monolithic contrast; compute-matched confirmation preferred. | partial; call budgets are frozen and M3/M4 are model-call-count matched, but runners and experiments do not exist |
...
| DoD-55 | Retain every expected method-case row, including failures and parse errors. | partial; the parser preserves raw `PARSE_ERROR` output, but no checkpointing runner exists |
...
work may continue. The call budget, strict structural parser, standalone
recursive grounding validator, and immutable Qwen revisions are now frozen.
The next non-review-dependent work is the correctness-first portion of CP-32
and CP-39: implement versioned method prompts/runners and row-level
checkpoint/resume without launching the full experiment. Model inference
remains blocked.
