...
| DoD-61 | Derive conclusions from observed results. | not evaluated; locked raw results exist but no study outcome has been scored |
...
admits all 11,360 rows without hidden-label access or scoring. The exact next
gate is registered label-separated deterministic scoring. Warm-up and thermal
controls remain separate blockers for the later resource experiment.
