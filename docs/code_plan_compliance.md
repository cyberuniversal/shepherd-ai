...
|---|---|---|---|
| CP-25 | Pass correctness and smoke gates before the full GPU run. | partial | Source, split, eligibility, alias, wiring, model-cache, synthetic-load, scoring, and unit-test gates exist. The commit-bound accuracy CLI adds a no-inference preflight; final configs and revised-study inference remain pending. |
...
| CP-36 | Separate one deterministic accuracy run from three resource repetitions on 30 clusters. | partial | `accuracy_case_manifest_v1.json` approves 284 held-out clusters and 1,420 cases for one deterministic accuracy run per model. The tested config builder and execution CLI enforce one commit-bound accuracy matrix per model without resource monitoring. Resource configs remain separate: `resource_schedule_candidate_v1.json` selects 30 tasks and registers 24 model-method-repetition conditions; warm-up and final hardware bindings remain resource-only blockers. |
...
| DoD-57 | Freeze protocol before inspecting final method scores. | implemented as score-blind registration pending clean-commit run binding; no study inference or score inspection exists |
...
orders are recorded without model invocation. An approval-gated builder is
ready to bind final run configs once these changes are committed. The reusable
accuracy CLI refuses inference until that binding, source-cleanliness, data,
protocol, cache, and expected-row preflight passes. That clean commit binding
is the only remaining accuracy-run gate. Warm-up and thermal
