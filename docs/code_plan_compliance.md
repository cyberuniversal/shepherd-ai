...
|---|---|---|---|
| CP-25 | Pass correctness and smoke gates before the full GPU run. | implemented for pre-inference accuracy gates | Source, split, eligibility, alias, wiring, model-cache, synthetic-load, scoring, unit-test, final config-binding, and both complete cache/runtime preflight gates pass. Revised-study inference has not started. |
...
| CP-36 | Separate one deterministic accuracy run from three resource repetitions on 30 clusters. | partial | `accuracy_case_manifest_v1.json` approves 284 held-out clusters and 1,420 cases for one deterministic accuracy run per model. Final configs bind one 5,680-row accuracy matrix per model to commit `419409e3bbfa8573b19b5c52cbfc4bdbc7d261e3`; both preflights pass without resource monitoring. Resource configs remain separate: `resource_schedule_candidate_v1.json` selects 30 tasks and registers 24 model-method-repetition conditions; warm-up and final hardware bindings remain resource-only blockers. |
...
| DoD-57 | Freeze protocol before inspecting final method scores. | implemented; protocol, scorer, manifest, and final model configs were bound before inference, and no study inference or score inspection exists |
