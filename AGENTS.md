...
- Parser-side target-slot normalization is allowed only for bounded intent fields such as known landmark aliases, coordinates, operator references, home/current-position, and unresolved ambiguous targets. It must not create MAVSDK commands or bypass deterministic backend safety.
