...
- Target schema migration is intentionally two-phase: Phase 1 keeps legacy `target_zone` while adding `target_raw_text`, `target_type`, and `target_resolution_required`; Phase 2 should migrate training/evaluation/runtime contracts to a nested `target` object after compatibility tests are in place.
