...
- Runtime learned-parser loading must be feature-flagged with `SHEPHERD_ENABLE_LEARNED_PARSER` or `SHEPHERD_PARSER_RUNTIME=learned`, and must verify a matching promoted parser report before use. Invalid artifacts or reports should fail closed to the existing parser path.
