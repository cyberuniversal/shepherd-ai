...
- Parser shadow reports must remain read-only over evidence records and must not switch runtime parser behavior or dispatch MAVSDK commands.
- Parser shadow candidate exports are review artifacts only. Keep generated candidates under ignored paths such as `.tmp_scenarios/`; do not append them to training data until a human selects or corrects `expected_intent`.
- Use `backend.parser_shadow_review` for reviewed candidate promotion. It should skip unreviewed candidates, require approved statuses, emit train-split dataset rows, and validate the output before it is used for augmentation.
...
- Runtime learned-parser loading must be feature-flagged with `SHEPHERD_ENABLE_LEARNED_PARSER` or `SHEPHERD_PARSER_RUNTIME=learned`, and must verify a matching promoted parser report before use. Use `SHEPHERD_SHADOW_LEARNED_PARSER=1` for report-only active-vs-learned parser audits. Invalid artifacts or reports should fail closed to the existing parser path.
- Parser-side intent-slot normalization is allowed only for bounded intent fields such as known landmark aliases, coordinates, operator references, home/current-position, unresolved ambiguous targets, explicit action verbs, mission patterns, priority words, and explicit drone counts. It must not create MAVSDK commands or bypass deterministic backend safety.
- Target resolution must use offline local gazetteers or explicit coordinates. Do not add online geocoding, random fallback coordinates, or model-memorized place-name dispatch. Unknown or ambiguous place names should block planning until resolved.
- Target schema migration is intentionally staged: legacy `target_zone` remains for compatibility, while runtime/training contracts also carry a nested `target` object plus `target_raw_text`, `target_type`, and `target_resolution_required`. Do not remove `target_zone` until frontend, evidence replay, parser promotion, and dataset migration tests all cover the nested target path.
...
- Use `backend.parser_comparison` to compare baseline and augmented artifacts on the same held-out scope before deciding whether a dataset expansion helped.
