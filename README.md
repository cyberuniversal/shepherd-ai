...
The current leading development objective is BCE-Dice. On the fixed seed-17 256/256 T4 split it reached modified mIoU `0.15387`; a subsequent train-label-stratified 256/256 CPU diagnostic reached `0.15617` at its selected checkpoint. The latter is only a small, non-device-matched improvement: drydown was the sole anomaly class with nonzero IoU. A matched T4 rerun is still required before adopting stratified sampling, and neither result is a final Agriculture-Vision benchmark.
